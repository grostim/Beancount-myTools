...
class PDFBourso(importer.ImporterProtocol):
    ...
    def extract(self, file, existing_entries=None):
        """
        Extrait les données du fichier PDF et les convertit en entrées comptables.
        
        :param file: Le fichier PDF à traiter
        :param existing_entries: Les entrées existantes (non utilisé dans cette implémentation)
        :return: Une liste d'entrées comptables
        """
    ...
    def _extract_dividende_bourse(self, file, text, document):
        """Extrait les données des dividendes boursiers."""
        entries = []
    ...
    def _extract_espece_bourse(self, file, text, document):
        """Extrait les données des espèces en bourse."""
        entries = []
    ...
    def _extract_bourse(self, file, text, document):
        """Extrait les données des opérations en bourse."""
        entries = []
        ...
        control = r"Montant transaction\s*Montant transaction brut\s*Intérêts\s*total brut\s*Courtages\s*Montant transaction net\s*(\d{0,3}\s*\d{1,3}[,.]\d{1,3})\s([A-Z]{3})\s*(\d{0,3}\s*\d{1,3}[,.]\d{1,3})\s([A-Z]{3})\s*(\d{0,3}\s*\d{1,3}[,.]\d{1,3})\s([A-Z]{3})\s*(\d{0,3}\s*\d{1,3}[,.]\d{1,3})\s([A-Z]{3})\s*"
        match = re.search(control, text)
        if match:
    ...
    def _extract_opcvm(self, file, text, document):
        """Extrait les données des opérations sur OPCVM."""
        entries = []
        # Identification du numéro de compte
        control = r"\d{5}\s\d{5}\s(\d{11})\s"
        match = re.search(control, text)
    ...
    def _extract_compte(self, file, text, document):
        """Extrait les données des comptes."""
        entries = []
        # Identification du numéro de compte
        control = r"\s*\d{11}"
        match = re.search(control, text)
